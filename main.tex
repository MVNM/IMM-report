\documentclass{article}

\input{preambule}

% \synthese{}
\mnemonic{INFO-H518}
\course{Immersive Multimedia Technologies I}
\thematic{Report}

\title{\texttt{\theMnemonic}: \theCourse \\ \texttt{\theThematic}}
\author{Mathieu Vannimmen \texttt{(543455)}}
\date{\texttt{\today}}

\begin{document}

\maketitle

{\hypersetup{linkcolor=black}
    \tableofcontents
}
\newpage

\section{First Part}\label{sec:firstpart}

    \input{sections/radialdistortion}

    \input{sections/homography}

    \input{sections/rectification}

    \input{sections/stereomatching}

\newpage
\section{Second Part}\label{sec:secondpart}

    \input{sections/viewsynthesis}

    \input{sections/iteratedclosestpoint}

    \input{sections/colmap}

    \input{sections/zhang}


\newpage
    \bibliography{refs}
    
\end{document}