...
A homography is a transformation of space that can model a translation, rotation and shear of a picture as well a
perspective change.
This is used in the context of panoramic stitching to take two pictures with a common section and stitch them
together (only if the two pictures are taken from the same position or if the photographied scene is a plane).
...
the two pictures of figure~\ref{fig:castleimages}, which seems plausible (the camera seems to have only tilded slightly
and changed orientation between the two shots, not moved in space).
...
The input image 1 and 2 are simply the input images.
...
This is because the authors decide to fix the second image and to warp the first one onto it: only the first image
is modified to build the panorama (\textit{QH.2}).

Importantly, just taking the average of these two registered images pixel-wise gives the output panorama.
...
\begin{figure}[H]
    ...
    \caption{(Simplified) code from~\citep{homography} apply ORSA, their special RANSAC\@.}
    \label{fig:orsa}
\end{figure}
